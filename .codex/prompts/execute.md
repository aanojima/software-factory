...
carry it out. This prompt is self-contained (Codex has no plugin system, so the
routing rules are embedded here).
...
core, stop at the commit — is also available on its own as `stage-ticket`
(`$stage-ticket`), when you want the branch staged but aren't ready to open
the PR yet.
