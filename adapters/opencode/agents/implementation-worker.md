---
description: Bounded repo-local implementation worker; the host owns orchestration and is the only writer owner for an implementation or repair turn
mode: subagent
permission:
  edit: allow
  bash: allow
...
resolved `TEST_LOOP_CAP=<value>` from
`.opencode/software-factory/loops.env`; use that literal value and do not make the target
repository resolve plugin configuration.
