...
2. Act on the classification:
   ...
   - `STANDARD` (tier T1): spawn `Agent({model: "$EXEC_MODEL", subagent_type: "general-purpose"})` with the event, the failing check's log tail or comment thread, the PR diff, and the fix recipe below. The classifier only saw one event line; the dispatched agent sees the real code, so it gets the final call on whether STANDARD's premise (this is a valid, actionable fix) actually holds — tell it explicitly that if it decides the suggestion doesn't apply (already handled, contradicts a repo convention it can name), it should reply on the thread with that reason instead of forcing a change, and report that outcome back distinctly from a completed fix.
   - `HEAVY` or risk=high: do not write code yourself. Spawn `conformance-reviewer` (baseline, always) plus any other `*-reviewer` agent whose description matches this finding (see `../skills/route/references/review-panel.md` — same selection rule the pre-PR panel uses, so a newly-added specialized reviewer applies here too with no change to this file), all read-only, in parallel. Then `SendMessage(to: "main", message: "...")` with the finding and every reviewer's assessment — this is always a human decision, and you have no other way to reach the human. Mark the event `awaiting_decision` in `state.json` so you don't re-triage or re-escalate it, then keep watching: don't stop for one pending decision, other events on this PR still need triage. When the decision comes back as an incoming message:
     - **apply** — spawn `Agent({model: "$EXEC_MODEL", subagent_type: "general-purpose"})` with the finding, every reviewer's assessment, the human's decision, and the fix recipe below — same dispatch as STANDARD, just with the reviewers' write-ups as extra context instead of a raw log tail.
